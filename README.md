...
**Шаг 1.** В Zed откройте глобальные настройки: меню `Zed -> Settings -> Open Settings File` (Mac) или `File -> Settings -> Open Settings File` (Windows/Linux). Откроется файл с текстом `{}`.

**Шаг 2.** Скопируйте блок ниже целиком, выделите все содержимое файла настроек и вставьте:
...
**Шаг 4.** Откройте хранилище: меню `File -> Open Folder` -> выберите папку, которую скачали на шаге 2.
...
**Шаг 6.** Вернитесь в Zed. Откройте панель AI-ассистента: меню `View -> Agent Panel`. В нижней части панели нажмите на название модели и выберите `DeepSeek V3.2 (ProxyAPI)`. Zed попросит ввести API-ключ - вставьте скопированный ключ. Ключ сохранится, повторно вводить не нужно.
