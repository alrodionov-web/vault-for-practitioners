...
**Шаг 1.** Откройте [proxyapi.ru](https://proxyapi.ru), создайте аккаунт и пополните баланс (от 100 руб, DeepSeek стоит ~0.5-2 руб за запрос). Скопируйте API-ключ (начинается с `sk-`).

**Шаг 2.** В Zed откройте глобальные настройки: меню `Zed -> Settings -> Open Settings` (Mac) или `File -> Settings -> Open Settings` (Windows/Linux). Откроется файл с текстом `{}`.

**Шаг 3.** Выделите все содержимое файла и замените на этот блок (скопируйте целиком):
...
**Шаг 4.** Сохраните файл (`Cmd+S` на Mac, `Ctrl+S` на Windows). Закройте и снова откройте Zed.

**Шаг 5.** Откройте хранилище: меню `File -> Open Folder` -> выберите папку, которую скачали на шаге 2.

**Шаг 6.** Откройте панель AI-ассистента: меню `View -> Agent Panel`. В нижней части панели нажмите на название модели и выберите `DeepSeek V3.2 (ProxyAPI)`. Zed попросит ввести API-ключ - вставьте ключ из шага 1. Ключ сохранится, повторно вводить не нужно.
